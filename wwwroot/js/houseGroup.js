...
var connectionHouse = new signalR.HubConnectionBuilder()
    ...
    .withUrl("/hubs/housegroup").build();  // User Transporttype can be configured to WebSockets, or ServerSentEvents or LongPolling
...
function fulfilled() {
    console.log("Connection to connectionHouseGroup Hub Successful");
}

function rejected() {
    console.log("Connection to connectionHouseGroup Hub Failed");
}
...
connectionHouse.on("newMemberAddedToHouse", (houseName) => {
    toastr.success(`New Member has joined ${houseName}`);
});

connectionHouse.on("newMemberRemovedFromHouse", (houseName) => {
    toastr.warning(`New Member has left ${houseName}`);
})
